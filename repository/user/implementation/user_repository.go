...
func NewAuthRepository(db *sqlx.DB) user.Repository {
...
func (r *authRepo) Register(ctx context.Context, user *models.User) (*models.User, error) {
	...
	u := &models.User{}
	if err := r.db.QueryRowxContext(ctx, createUserQuery, &user.Email, &user.Active, &user.Email).StructScan(u); err != nil {
...
func (r *authRepo) Update(ctx context.Context, user *models.User) (*models.User, error) {
	...
	u := &models.User{}
	if err := r.db.GetContext(ctx, u, updateUserQuery, &user.Email, &user.Active, &user.Email); err != nil {
